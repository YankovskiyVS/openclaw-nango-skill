...
> **Required pip:** `httpx`  
> **Install only if** this EvoClaw has OAuth connection for `bitrix24-user` in Cloud.ru console.


## What this skill does

**Bitrix24 Users & Structure** — authenticated HTTP via **ai-assistant-nango-proxy** → Nango → provider API.

- Nango `provider_config_key`: **`bitrix24-user`**
- Scopes / access: `user, department`
...
User asks about Bitrix24 employees, departments, org chart.

Do **not** use for other vendors — install the matching skill (`yandex-*`, `bitrix24-*`, `amocrm-*`).

## Prerequisites

1. User completed OAuth for **`bitrix24-user`** on this EvoClaw in Cloud.ru console.
...
1. Confirm the request matches **Bitrix24 Users & Structure** (`bitrix24-user`).
2. Prefer `python3 {baseDir}/scripts/nango_proxy.py call bitrix24-user …`.
3. On **401** — API key / IAM; do not invent alternate auth.
4. On **404** — wrong `EVOCLAW_ID`.
5. On upstream **4xx/5xx** — missing/expired OAuth → ask user to reconnect **bitrix24-user** in console.
