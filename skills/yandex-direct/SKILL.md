...
**Yandex Direct** — authenticated HTTP via **ai-assistant-nango-proxy** → Nango → provider API.
...
User asks about Yandex Direct campaigns, ads, or reports.

Do **not** use for other vendors — install the matching skill (`yandex-*`, `bitrix24-*`, `amocrm-*`).

## Prerequisites

1. User completed OAuth for **`yandex-direct`** on this EvoClaw in Cloud.ru console.
...
1. Confirm the request matches **Yandex Direct** (`yandex-direct`).
2. Prefer `python3 {baseDir}/scripts/nango_proxy.py call yandex-direct …`.
3. On **401** — API key / IAM; do not invent alternate auth.
4. On **404** — wrong `EVOCLAW_ID`.
5. On upstream **4xx/5xx** — missing/expired OAuth → ask user to reconnect **yandex-direct** in console.
