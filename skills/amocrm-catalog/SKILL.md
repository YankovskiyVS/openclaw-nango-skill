...
> **Required pip:** `httpx`  
> **Install only if** this EvoClaw has OAuth connection for `amocrm-catalog` in Cloud.ru console.


## What this skill does

**amoCRM Catalog & Purchases** — authenticated HTTP via **ai-assistant-nango-proxy** → Nango → provider API.
...
User asks about amoCRM products, catalogs, purchases.

Do **not** use for other vendors — install the matching skill (`yandex-*`, `bitrix24-*`, `amocrm-*`).

## Prerequisites

1. User completed OAuth for **`amocrm-catalog`** on this EvoClaw in Cloud.ru console.
...
1. Confirm the request matches **amoCRM Catalog & Purchases** (`amocrm-catalog`).
2. Prefer `python3 {baseDir}/scripts/nango_proxy.py call amocrm-catalog …`.
3. On **401** — API key / IAM; do not invent alternate auth.
4. On **404** — wrong `EVOCLAW_ID`.
5. On upstream **4xx/5xx** — missing/expired OAuth → ask user to reconnect **amocrm-catalog** in console.
