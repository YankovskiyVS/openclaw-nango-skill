...
> **Required pip:** `httpx`  
> **Install only if** this EvoClaw has OAuth connection for `bitrix24-im` in Cloud.ru console.

## What this skill does

**Bitrix24 Messenger** — authenticated HTTP via **ai-assistant-nango-proxy** → Nango → provider API.
...
User asks to send Bitrix24 chat messages, manage chatbots, or open lines.

Do **not** use for other vendors — install the matching skill (`yandex-*`, `bitrix24-*`, `amocrm-*`).

## Prerequisites

1. User completed OAuth for **`bitrix24-im`** on this EvoClaw in Cloud.ru console.
...
1. Confirm the request matches **Bitrix24 Messenger** (`bitrix24-im`).
2. Prefer `python3 {baseDir}/scripts/nango_proxy.py call bitrix24-im …`.
3. On **401** — API key / IAM; do not invent alternate auth.
4. On **404** — wrong `EVOCLAW_ID`.
5. On upstream **4xx/5xx** — missing/expired OAuth → ask user to reconnect **bitrix24-im** in console.
